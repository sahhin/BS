 ...
 * 04.02.2019 Silke Behn TFT_putc/ TFT_puts  wenn ein # kommt, doppeltes # ausgeben, damit ein # angezeigt wird
 *            Neue Funktionen: TFT_set_text_color /  TFT_FillArea / TFT_bigChar / TFT_bigString
 */
 ...
 * x2,y2 rechts unten
   ---------------------------------------------------------------------------
*/
void TFT_FillArea(int x1, int y1, int x2, int y2, char color){
   ...
   cmd[9] = y2 & 0xFF;
	 ...
	 TFT_send_data(12, cmd);	 
	
}


/*------------------------------------------------------------------------------------
  * Silke Behn  05.02.2019
  * Löscht einen Bereich vom Display
  * ----------------------------------------------------------------------------------
**/
void TFT_DeleteArea(int x1, int y1, int x2, int y2){
...
/**---------------------------------------------------------------------------------------
  ...
  * --------------------------------------------------------------------------------------
**/
void TFT_bigString(int x, int y, char * str, char color){
   int i=0;
	 char cmd[9];
	 ...
	 if ( (x>=0) && (x<=12) && (y>=0) && (y<=7) ) {
	    ...
	    while ( (str[i] != '\0') && ( y <= 7) ) {
		      ...
		      //Löscht das alte Zeichen 
			    TFT_DeleteArea(x*25, y*29, (x*25)+25, (y*29)+29);
          ...
          TFT_send_data(9, cmd);
	        ...
	        i += 1;
          x += 1;		
	        //Es passen nur 12 Zeichen in eine Zeile	
	        if ( x > 12 ) {
             x = 0;
             y += 1;
          }		   
...
/**--------------------------------------------------------------------------------------
  ...
  * Breite eines Zeichen = 25 =>  0 <= x <=12
  * Höhe eines Zeichen   = 29 =>  0 <= y <=7  
  *--------------------------------------------------------------------------------------
**/
void TFT_bigChar(int x, int y, char b, char color){
   char cmd[9];
...
/**
  ...
  * @retval none
  */
void TFT_set_text_color(int color){
